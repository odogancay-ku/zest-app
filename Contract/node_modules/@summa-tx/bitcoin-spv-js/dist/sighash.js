...
exports.parseVout = parseVout;
exports.hashPrevouts = hashPrevouts;
exports.hashSequence = hashSequence;
exports.hashOutputs = hashOutputs;
exports.possibleAbsoluteLock = possibleAbsoluteLock;
exports.possibleRelativeLock = possibleRelativeLock;
exports.sighash = sighash;
exports.deserSighashArgs = deserSighashArgs;
exports.serSighashObj = serSighashObj;
exports.rpcSighash = rpcSighash;
exports.U32_MAX = exports.NULL_HASH = void 0;
...
function hashPrevouts(inputs, flag) {
  if ((flag & 0x80) === 0x80) {
    return NULL_HASH;
  }

  const preimage = utils.concatUint8Arrays(...inputs.map(BTCUtils.extractOutpoint));
...
function hashSequence(inputs, flag) {
  if ((flag & 0x80) === 0x80 || (flag & 0x03) === 0x03) {
    return NULL_HASH;
  }

  const preimage = utils.concatUint8Arrays(...inputs.map(BTCUtils.extractSequenceLELegacy));
  return BTCUtils.hash256(preimage);
}
/**
 *
...
function possibleAbsoluteLock(inputs, locktime, flag) {
  if ((flag & 0x80) === 0x80) return true;
  const sequences = inputs.map(BTCUtils.extractSequenceLegacy); // if all sequences are UINT_MAX, no locktime is possible
...
function possibleRelativeLock(inputs, version) {
...
function sighash(tx, index, sighashFlag, prevoutScript, prevoutValue) {
  ...
  const preimage = utils.concatUint8Arrays(tx.version, hashPrevouts(inputs, sighashFlag), hashSequence(inputs, sighashFlag), BTCUtils.extractOutpoint(currentInput), prevoutScript, prevoutValue, BTCUtils.extractSequenceLELegacy(currentInput), hashOutputs(outputs), tx.locktime, new Uint8Array([sighashFlag, 0, 0, 0]) // sighashFlag as LE u32
  );
  return {
...
function deserSighashArgs(serTx, serPrevoutScript, serPrevoutValue) {
  const tx = {};
...
function serSighashObj(sighashObj) {
  return {
