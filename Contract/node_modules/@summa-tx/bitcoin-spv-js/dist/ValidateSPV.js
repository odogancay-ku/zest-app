...
exports.validateHeaderWork = validateHeaderWork;
exports.validateHeaderPrevHashLE = validateHeaderPrevHashLE;
exports.validateHeaderChain = validateHeaderChain;
 ...
 */
function prove(txid, merkleRoot, intermediateNodes, index) {
  // Shortcut the empty-block case
  if (utils.typedArraysAreEqual(txid, merkleRoot) && index === 0 && intermediateNodes.length === 0) {
    return true;
  }

  const proof = utils.concatUint8Arrays(txid, intermediateNodes, merkleRoot); // If the Merkle proof failed, bubble up error
...
function validateHeaderWork(digest, target) {
...
function validateHeaderPrevHashLE(header, prevHeaderDigest) {
  // Extract prevHash of current header
  const prevHashLE = BTCUtils.extractPrevBlockLE(header); // Compare prevHash of current header to previous header's digest
...
function validateHeaderChain(headers) {
  ...
  for (let i = 0; i < headers.length / 80; i += 1) {
    ...
    const start = i * 80;
    const header = utils.safeSlice(headers, start, start + 80); // After the first header, check that headers are in a chain
    ...
    const target = BTCUtils.extractTarget(header); // Require that the header has sufficient work
