...
exports.determineVarIntDataLength = determineVarIntDataLength;
exports.parseVarInt = parseVarInt;
exports.hash160 = hash160;
exports.hash256 = hash256;
exports.extractScriptSigLen = extractScriptSigLen;
exports.extractSequenceLELegacy = extractSequenceLELegacy;
exports.determineInputLength = determineInputLength;
exports.extractInputAtIndex = extractInputAtIndex;
exports.isLegacyInput = isLegacyInput;
exports.extractSequenceLegacy = extractSequenceLegacy;
exports.extractScriptSig = extractScriptSig;
exports.extractSequenceLEWitness = extractSequenceLEWitness;
exports.extractSequenceWitness = extractSequenceWitness;
exports.extractOutpoint = extractOutpoint;
exports.extractInputTxIdLE = extractInputTxIdLE;
exports.extractTxIndexLE = extractTxIndexLE;
exports.extractTxIndex = extractTxIndex;
exports.determineOutputLength = determineOutputLength;
exports.extractOutputAtIndex = extractOutputAtIndex;
exports.extractValueLE = extractValueLE;
exports.extractValue = extractValue;
exports.extractOpReturnData = extractOpReturnData;
exports.extractHash = extractHash;
exports.validateVin = validateVin;
exports.validateVout = validateVout;
exports.extractMerkleRootLE = extractMerkleRootLE;
exports.extractTarget = extractTarget;
exports.calculateDifficulty = calculateDifficulty;
exports.extractPrevBlockLE = extractPrevBlockLE;
exports.extractTimestampLE = extractTimestampLE;
exports.extractTimestamp = extractTimestamp;
exports.extractDifficulty = extractDifficulty;
exports.hash256MerkleStep = hash256MerkleStep;
exports.verifyHash256Merkle = verifyHash256Merkle;
exports.retargetAlgorithm = retargetAlgorithm;
exports.DIFF_ONE_TARGET = exports.RETARGET_PERIOD_BLOCKS = exports.RETARGET_PERIOD = void 0;
 ...
 * @const {BigInt}
 */
const RETARGET_PERIOD = BigInt(1209600);
/**
 * @const {BigInt}
 */

exports.RETARGET_PERIOD = RETARGET_PERIOD;
const RETARGET_PERIOD_BLOCKS = BigInt(2016);
/**
 * @const {BigInt}
 */

exports.RETARGET_PERIOD_BLOCKS = RETARGET_PERIOD_BLOCKS;
const DIFF_ONE_TARGET = BigInt('0xffff0000000000000000000000000000000000000000000000000000');
/**
 *
...
exports.DIFF_ONE_TARGET = DIFF_ONE_TARGET;

function determineVarIntDataLength(flag) {
  if (flag === 0xff) {
...
function extractScriptSigLen(input) {
  if (input.length < 37) {
  ...
  const {
    dataLength,
    number
  } = parseVarInt(utils.safeSlice(input, 36));
  return {
    dataLength,
    scriptSigLen: number
  };
...
function extractSequenceLELegacy(input) {
  const {
    dataLength,
    scriptSigLen
...
function determineInputLength(input) {
  const {
    dataLength,
    scriptSigLen
  } = extractScriptSigLen(input);
...
function extractInputAtIndex(vin, index) {
  const {
    dataLength,
    number: nIns
  } = parseVarInt(vin);
...
function extractSequenceLegacy(input) {
...
function extractScriptSig(input) {
  const {
    dataLength,
    scriptSigLen
  } = extractScriptSigLen(input);
...
function extractSequenceLEWitness(input) {
...
function extractSequenceWitness(input) {
...
function extractOutpoint(input) {
...
function extractInputTxIdLE(input) {
...
function extractTxIndexLE(input) {
...
function determineOutputLength(output) {
  if (output.length < 9) {
  ...
  const {
    dataLength,
    number
  } = parseVarInt(utils.safeSlice(output, 8)); // 8 byte value, 1 byte for len itself
...
function extractOutputAtIndex(vout, index) {
  const {
    dataLength,
    number: nOuts
  } = parseVarInt(vout);
...
function extractValueLE(output) {
...
function validateVin(vin) {
  try {
    ...
    const {
      dataLength,
      number: nIns
    } = parseVarInt(vin); // Not valid if it says there are too many or no inputs
...
function validateVout(vout) {
  try {
    ...
    const {
      dataLength,
      number: nOuts
    } = parseVarInt(vout); // Not valid if it says there are too many or no inputs
...
function extractMerkleRootLE(header) {
...
function extractPrevBlockLE(header) {
...
function extractTimestampLE(header) {
...
function extractTimestamp(header) {
  return utils.bytesToUint(utils.reverseEndianness(extractTimestampLE(header)));
}
/**
 *
...
function extractDifficulty(header) {
...
function hash256MerkleStep(a, b) {
...
function verifyHash256Merkle(proof, index) {
  let idx = index;
  const proofLength = proof.length; // Not an even number of hashes
...
function retargetAlgorithm(previousTarget, firstTimestamp, secondTimestamp) {
  let elapsedTime = BigInt(secondTimestamp - firstTimestamp);
  const rp = RETARGET_PERIOD;
  const lowerBound = rp / BigInt(4);
  const upperBound = rp * BigInt(4); // Normalize ratio to factor of 4 if very long or very short
